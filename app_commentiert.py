...
# read data from file
data = pd.read_excel(r"/home/zabi/Schreibtisch/Data_Visualization/Data_Visualization-dashboard_side_1/Data_Visualization/data/data.xlsx")
...
# calculating "Delivery deviation in days" column
data["Delivery deviation in days"] = data["delivery date"] - data["supplier delivery date"]
# changing the format of dates in each row to float numbers (for example 0 days--> 0.0 )
data["Delivery deviation in days"] = data["Delivery deviation in days"].apply(lambda x: float(x.days))

# conditions for the "Buckets" (siehe Aufgabenstellung)
...
# the Values for the "Buckets"
values = ['in time', 'late: < 5 days', 'late: 5 to 10 days', 'late: > 10 days']
# calculating the 'deviation indicator' column under the conditions above
data['deviation indicator'] = np.select(conditions, values)

# filling the columns "Year/Month", "Year", "Month" (see data table)
...
"""Task: Gesamtsumme Ordered Spend & Anzahl Bestellungen (aktuel vs vorjahr)"""
# sum of net value according to year (2019, 2020)
orderedSpend_year = data.groupby(["Year"])[["Net Value"]].sum().reset_index()
# two dataframes for 2019 and 2020--> need it later
data_2019 = data[data["Year"] == 2019]
data_2020 = data[data["Year"] == 2020]

# calculating number of orders for 2019
numbOrders_2019 = len(pd.unique(data_2019["Purchasing Doc."]))
# calculating number of orders for 2020
numbOrders_2020 = len(pd.unique(data_2020["Purchasing Doc."]))

"""Task: Ordered Spend & Anzahl Bestellungen im aktuellen Jahr (YTD) nach Monaten
im Vergleich zum Vorjahr:"""
# calculating net value according to months in 2019 and 2020
...
# counting unique purchasing Doc. numbers based on "Month" --> to get number of orders per month
...
"""Task: Ordered Spend & Anzahl Bestellungen im aktuellen Jahr (YTD) nach
Einkaufsorganisation im Vergleich zum Vorjahr"""

# similar process as last task, this time filtering based on Purchasing Org. 
...
numbOrders_purchOrg_2020 = df2020_without_purchDoc_duplicates.groupby(
    ["Purchasing Org."])[["Purchasing Doc."]].count().reset_index().rename(columns={"Purchasing Doc.": "numbOrders"})
# zum string umwandeln damit plotly die zahlen nicht als zahl sondern als eine Bezeichnung für eine Organistan (purchasing Org) erkennt
...
"""Task: Top 10 Lieferanten nach Ordered Spend (aktuelles Jahr) im Vergleich zum
Vorjahr"""
...
def update_chart_cc(value):
    ...
    print(company_code)
    print(all_cc)
    benoetigte_cc = [int(i) for i in all_cc if i in company_code]
...
def update_charts(os_nof_value, cc_value, mg_value):
